...
async function setupServer() {
  ...
  server.post<{
    Body: TwiMLRequest
  }>('/call-twiml', async (request, reply) => {
    try {
  ...
  server.get('/call-stream', { websocket: true }, (connection, request) => {
    ...
    twilioSocket.on('message', (rawMessage: WebSocket.Data) => {
      try {
        ...
        if (message.event === 'start') {
          ...
          hasStarted = true;
          elevenLabsClient.startConversation().catch((err: Error) => {
          ...
          if (payload && activeConnections.has(streamSid)) {
            ...
            if (connection) {
              connection.elevenLabsClient.sendAudio(payload).catch((err: Error) => {
    ...
    twilioSocket.on('close', (code: number, reason: string) => {
    ...
    function cleanupConnection(sid: string) {
      console.log(`Cleaning up connection for stream ${sid}`);
      const connection = activeConnections.get(sid);
      
      if (connection) {
        try {
          connection.elevenLabsClient.endConversation().catch((err: Error) => {
